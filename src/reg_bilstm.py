...
class RegressionBilstmModel(object):
    def __init__(self, opt="origin"):
        ...
        logging.debug("emoji_embedding_matrix shape: {}".format(self.emoji_embedding_matrix.shape))
    ...
    def build_model(self):
        ...
        model.add(Bidirectional(LSTM(units=self.lstm_size), merge_mode="concat"))

        model.add(Dense(units=self.embedding_dim, activation="linear"))

        # compile model
        model.compile(loss="cosine_proximity",  # mean_squared_error, cosine_distance, cosine_proximity
                      optimizer="adam",
                      metrics=["mse"])
    ...
    def train_model(self):
        ...
        self.model.fit(self.x_train, self.y_train,
                       ...
                       verbose=2,
                       callbacks=[mac_f1, early_stopping])
        stop = time.time()
