...
class ClassificationBilstmModel(object):
    def __init__(self, opt="origin", pretrained_embedding=True):
        ...
        self.experiment_name = "cls_bilstm"
    ...
    def train_model(self):
        ...
        checkpoint_filepath = "output/models/" + self.experiment_name + ".weights.{epoch:02d}-{val_loss:.2f}.hdf5"
        checkpoint_filepath = checkpoint_filepath
        ckpt = ModelCheckpoint(checkpoint_filepath)
        start = time.time()
        self.model.fit(self.x_train, self.y_train,
                       ...
                       verbose=2,
                       callbacks=[mac_f1, early_stopping, ckpt])
        stop = time.time()
